...
/** Base class for builders of {@link AradSocket}. */
public abstract class AradSocketBuilder<T extends AradSocketBuilder<T>> {
  ...
  BufferAllocator plainBufferAllocator = AradSocket.defaultPlainBufferAllocator;
  BufferAllocator encryptedBufferAllocator = AradSocket.defaultEncryptedBufferAllocator;
  boolean releaseBuffers = true;
  boolean waitForCloseConfirmation = false;

  AradSocketBuilder(ByteChannel underlying) {
  ...
  /**
   * Whether to wait for TLS close confirmation when executing a local {@link AradSocket#close()} on
