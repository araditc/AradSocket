...
import com.araditc.aradsocket.impl.AradSocketImpl;
import com.araditc.aradsocket.impl.BufferHolder;
import com.araditc.aradsocket.impl.ByteBufferSet;
import com.araditc.aradsocket.impl.TlsExplorer;
import com.araditc.aradsocket.impl.AradSocketImpl.EofException;

/** A server-side {@link AradSocket}. */
public class AradSocketChannel implements AradSocket {

  private static final Logger logger = LoggerFactory.getLogger(AradSocketChannel.class);
  ...
  private static class SniSslContextStrategy implements SslContextStrategy {
    ...
    public SSLContext getSslContext(SniReader sniReader) throws IOException, EofException {
      ...
      try {
        ...
        throw new AradSocketCallbackException("SNI callback failed", e);
      }
  ...
  /** Builder of {@link AradSocketChannel} */
  public static class Builder extends AradSocketBuilder<Builder> {

    private final SslContextStrategy internalSslContextFactory;
    private Function<SSLContext, SSLEngine> sslEngineFactory =
        AradSocketChannel::defaultSSLEngineFactory;
    ...
    public AradSocketChannel build() {
      return new AradSocketChannel(
          underlying,
   ...
   * Due to limitations of {@link SSLEngine}, configuring a {@link AradSocketChannel} to select the
  ...
  private AradSocketImpl impl = null;

  // @formatter:off
  private AradSocketChannel(
    ...
    inEncrypted =
        new BufferHolder(
            ...
            AradSocketImpl.buffersInitialSize,
            AradSocketImpl.maxTlsPacketSize,
            false /* plainData */,
            releaseBuffers);
  }
  ...
  public long read(ByteBuffer[] dstBuffers, int offset, int length) throws IOException {
    ...
    AradSocketImpl.checkReadBuffer(dest);
    if (!sniRead) {
      try {
        initEngine();
    ...
    try {
      if (!sniRead) {
        ...
        try {
          ...
          throw new AradSocketCallbackException("SSLEngine creation callback failed", e);
        }
        impl =
            new AradSocketImpl(
                underlying,
    ...
    try {
      ...
      while (true) {
        try {
          ...
          AradSocketImpl.readFromChannel(underlying, inEncrypted.buffer); // IO block
        }
      }
    } finally {
      inEncrypted.release();
