...
import com.araditc.aradsocket.NeedsReadException;
import com.araditc.aradsocket.NeedsTaskException;
import com.araditc.aradsocket.NeedsWriteException;
import com.araditc.aradsocket.impl.ByteBufferSet;
import com.araditc.aradsocket.util.Util;

/**
 * This class encapsulates the infrastructure for running {@link AsynchronousAradSocket}s. Each
 ...
 */
public class AsynchronousAradSocketGroup {

  private static final Logger logger = LoggerFactory.getLogger(AsynchronousAradSocketGroup.class);
  ...
  class RegisteredSocket {
    ...
    RegisteredSocket(AradSocket aradSocket, SocketChannel socketChannel) {
   ...
   *     completion handlers.
   */
  public AsynchronousAradSocketGroup(int nThreads) {
    try {
  ...
  public AsynchronousAradSocketGroup() {
    this(Runtime.getRuntime().availableProcessors());
  }

  RegisteredSocket registerSocket(AradSocket reader, SocketChannel socketChannel)
  ...
  private void writeHandlingTasks(RegisteredSocket socket, WriteOperation op) throws IOException {
    while (true) {
      try {
        socket.aradSocket.write(op.bufferSet.array, op.bufferSet.offset, op.bufferSet.length);
        return;
  ...
  private long readHandlingTasks(RegisteredSocket socket, ReadOperation op) throws IOException {
    while (true) {
      try {
        return socket.aradSocket.read(op.bufferSet.array, op.bufferSet.offset, op.bufferSet.length);
      } catch (NeedsTaskException e) {
        warnAboutNeedTask();
        e.getTask().run();
      }
